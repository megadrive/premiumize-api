...
export class PremiumizeClient {
  ...
  /**
   * Whether or not to validate API responses.
   */
  ...
  private async request<T>(opts: RequestArguments): Promise<T> {
    ...
    try {
      ...
      if (this.validateResponses && opts.schema) {
   ...
   */
  deleteTransfer(opts: P.DeleteTransfersRequest): Promise<P.DeleteTransfersResponse> {
